...
| Session code review (`/review-session`, `/review-message`) | Native | PSM code-review mechanism → native Pager modal: left file list (flat or tree), right BlockViewer edit preview (default filter = changes only). F2 `review_file_tree` default **off** (persisted); modal `t` toggles tree (cwd-relative, compact Java packages with `.`). `/review-message` reuses jump ListOverlay. No Pi custom UI. |
